...
namespace Lab6.Service
{
    public class BlockChainService
    {
        ...
        public decimal BaseFeePerByte { get; set; } = 0.005m;
        ...
        public void AddTransaction(Transaction transaction)
        {
            ...
            decimal currentFee = GetCurrentNetworkFee();
            decimal requiredFee = transaction.Size * currentFee;

            Transaction? oldTx = null;
            if (transaction.ReplacesTxId != null)
            {
                oldTx = PendingTransactions.FirstOrDefault(tx => tx.Id == transaction.ReplacesTxId);

                if (oldTx == null)
                {
                    throw new InvalidOperationException("Old tx not found!");
                }
                if (oldTx.From != transaction.From)
                {
                    throw new InvalidOperationException("Sender error! Only sender can change transaction!");
                }
                if (transaction.Fee <= oldTx.Fee)
                {
                    throw new InvalidOperationException("New fee should be higher than old one!");
                }
            }
            ...
            if (transaction.From != "COINBASE")
            {
                ...
                if (oldTx != null)
                {
                    senderBalance += (oldTx.Amount + oldTx.Fee);
                }

                if (senderBalance < transaction.Amount + transaction.Fee)
                {
                    throw new InvalidOperationException($"Insufficient balance for address {transaction.From}. Available: {senderBalance}, Required: {transaction.Amount + transaction.Fee}");
                }
                if (transaction.Fee < requiredFee)
                {
                    throw new InvalidOperationException($"Transaction fee is too low. Minimum required: {requiredFee}");
                }
            }

            if (oldTx != null)
            {
                PendingTransactions.Remove(oldTx);
                Console.WriteLine($"Transaction Id {oldTx.Id} was replaced on id {transaction.Id}.");
            }
        ...
        {
            int currentMempoolSize = PendingTransactions.Sum(tx => tx.Size);

            if (currentMempoolSize <= MaxBlockSizeBytes)
                return BaseFeePerByte;

            decimal congestionFactor = (decimal)currentMempoolSize / MaxBlockSizeBytes;
            int multiplier = (int)Math.Ceiling(congestionFactor);
